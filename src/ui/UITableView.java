...
abstract public class UITableView<T> extends UIView {
        ...
        if(dataSource != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                try {
                    ...
                    System.err.println("Something went wrong at "+i);
                }
            }
    ...
    private <T>Callback getCallback(TableColumnValue<T> value){
        return  (Callback<TableColumn.CellDataFeatures<T, String>, ObservableValue<String>>) param -> {
            String val = "null";
            try { val = value.value(param.getValue()).toString(); } catch (NullPointerException e){};
